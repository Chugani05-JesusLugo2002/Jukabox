...
import ArtistLabel from './ArtistLabel.vue';
...
<template>
    <ul class="list-group" v-if="result.result.hasOwnProperty('artists')">
      <ItemList v-for="artist in result.result.artists" :key="artist.id" :id="artist.id" :img="artist.avatar" :type="'artist'" :lbzUrl="artist.lbz_url">
    ...
    <ul class="list-group" v-if="result.result.hasOwnProperty('albums')">
      <ItemList v-for="album in result.result.albums" :key="album.id" :id="album.id" :img="album.cover" :type="'album'" :lbzUrl="album.lbz_url">
    ...
    <ul class="list-group" v-if="result.result.hasOwnProperty('songs')">
      <ItemList v-for="song in result.result.songs" :key="song.id" :id="song.id" :img="song.cover" :type="'song'" :lbzUrl="song.lbz_url">
