...
async function searchInAPI() {
    const result = 
        query.value ? await getExplorerResult(query.value, type.value) : ''
...
<template>
    ...
    <div class="d-flex justify-content-center align-items-center">
        <FilterPill v-for="(filter, index) in POSSIBLE_FILTERS" :key="index" @click="type=filter.id" :selected="filter.id == type">
            {{ filter.name }}
        </FilterPill>
    </div>
</template>