...
<template>
  <div class="mb-3">
    <a
      v-if="lbz_link"
      :href="lbz_link"
      class="btn btn-light w-100 py-3 border fw-bold"
      target="_blank"
    >
    ...
    <div class="d-flex flex-column mt-3" v-if="fixedLinks.length > 0">
      ...
      <div v-for="url in fixedLinks">
        <a
          :href="url.target"
          class="btn btn-light py-2 my-1 w-100 fw-bold border d-flex justify-content-center align-items-center"
          target="_blank"
        >
          ...
          <img
            v-else-if="url.urlType == 'Deezer'"
            src="/imgs/deezer-logo.svg"
            style="height: 32px"
            class="mx-2"
          />
          <img
            v-else-if="url.urlType == 'Bandcamp'"
