...
const { id, title, likes, reviews, type } = defineProps<{
    id: number
    title: string
...
<template>
    <RouterLink class="row text-reset text-decoration-none d-flex align-items-center rounded" :to="`/${type}/${id}`">
...
.row {
    transition: 0.3s;
    &:hover {
        background-color: rgb(226, 220, 207);
    }
}
</style>